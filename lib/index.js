...
const SEQUENCE_ASK_LIMIT = 5;
const SESSION_ASK_CAP = 5;
const PER_HOSTNAME_ASK_CAP = 1;
...
class PageReloadMonitor{
  constructor(){
    this.seqAskProbability = PageReloadMonitor.setAskProbabilities(SEQUENCE_ASK_LIMIT);
  ...
  shouldAsk(seqPos, hostname){
    ...
    return this.alwaysAsk() || (this.sampleAskProbability(seqPos) 
                            && !this.sessionAskCap() 
  ...
  incrementHostnameAsks(hostname){
    console.log(`incrementing hostname asks for ${hostname}`);
    ...
    console.log(`ask count for ${hostname}: `, storage.hostnameAsks.get(hostname));
  }
  ...
  listenForReload(){
    ...
    new WindowTracker({
        ...
        wd.get().addEventListener('keydown', (e)=>{

          let isDarwin = require('sdk/system').platform == 'darwin';
        ...
        reloadBtn.get().addEventListener('click', (e)=>{
  ...
  updatePageAttribute(tab, attribute, value){
    ...
    console.log(`new ${attribute}: ${value}`);
  }
  ...
  injectPageAgent(){
    ...
    require('sdk/page-mod').PageMod({
      ...
      onAttach: function onAttach(worker) {
        ...
        worker.port.on('page-title', (title) => {
          that.updatePageAttribute(worker.tab, 'page-title', title);
        });
      }
    });
  }
  ...
  reload(method, modKeys){
    ...
    data.sequence.createInstance(method, modKeys);
    data.sequence.reportLastInstance();

    console.log(data);

    if (this.shouldAsk(data.sequence.length, data.sequence.hostname))
      this.ask(data.sequence.hostname);
  }

  ask(hostname) {
    console.log(`asking: ${hostname}`);
