...
let study = null; //initialized in PageReloadMonitor's constructor

if (storage.hostnameAsks)
  log(storage.hostnameAsks['www.google.ca']);
...
class PageReloadMonitor{
  ...
  hostnameAskCap(hostname){
    if (!storage.hostnameAsks || !storage.hostnameAsks[hostname]) 
      return false;

    log(`hostname ask count for ${hostname}: ${storage.hostnameAsks[hostname]}`);

    return (storage.hostnameAsks[hostname] >= PER_HOSTNAME_ASK_CAP);
  }
  ...
  incrementHostnameAsks(hostname){
    ...
    if (!storage.hostnameAsks){
      storage.hostnameAsks = {};
      storage.hostnameAsks[hostname] = 1;
      return;
    }


    if (!storage.hostnameAsks[hostname]){
      log(`setting new hostname ask for ${hostname}`);
      storage.hostnameAsks[hostname] = 1;
      return;
    }
  ...
  sessionAskCap(){
    log(`session asked: ${this.sessionAsked}`);
  ...
  updatePageAttribute(tab, attribute, value){
    if (!this.tabData.get(tab)) return;
  ...
  openAbout(){
    tabs.open(self.data.url('html/about.html'));
  }
  ...
  popPanel(reactionCallback){
   ...
   cleanup(){
    ...
    Reflect.deleteProperty(storage, 'hostnameAsks');
  }
